...
const updCars = async(make, model, year, id) => {
  const connection = await mysql.createConnection(config);
  const qr = 'UPDATE cars SET make = ?, model = ?, year = ? WHERE id = ?';
  const [rows, fields] = await connection.execute(qr, [make, model, year, id]);
  connection.end();
  return rows;
}
